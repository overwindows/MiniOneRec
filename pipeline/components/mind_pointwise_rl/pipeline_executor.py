...
"""
Pipeline执行器 - Pointwise RL 训练版本

使用方法:
    python pipeline_executor.py --config config_mind_pointwise_rl.json --mount-dir /mnt/... --sft-model ... ...
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Pipeline执行器 - Pointwise RL 训练版本"
    )
    ...
    parser.add_argument("--sft-model", help="SFT checkpoint 路径（相对于挂载路径）")
    ...
    parser.add_argument("--neg-ratio", help="负样本比例（必须与 SFT 训练一致）")
    parser.add_argument("--use-abstract", help="是否使用新闻摘要 (1/0)")
    parser.add_argument("--use-chat-template", help="是否使用 chat template (1/0)")
    parser.add_argument("--regenerate-data", help="是否强制重新生成 parquet 数据 (1/0)")
    ...
    # Pointwise RL 参数 — always set to ensure template variables resolve
    cli_variables['reward_type'] = args.reward_type or "pointwise_asymmetric"
    cli_variables['max_response_length'] = args.max_response_length or "8"
    cli_variables['max_prompt_length'] = args.max_prompt_length or "4096"
    cli_variables['train_batch_size'] = args.train_batch_size or "128"
    cli_variables['learning_rate'] = args.learning_rate or "1e-7"
    cli_variables['kl_loss_coef'] = args.kl_loss_coef or "0.1"
    cli_variables['total_epochs'] = args.total_epochs or "1"
    cli_variables['num_generations'] = args.num_generations or "8"
    cli_variables['max_history'] = args.max_history or "30"
    cli_variables['neg_ratio'] = args.neg_ratio or "2.0"
    cli_variables['use_abstract'] = args.use_abstract or "1"
    cli_variables['use_chat_template'] = args.use_chat_template or "1"
    cli_variables['regenerate_data'] = args.regenerate_data or "0"
    cli_variables['run_eval'] = args.run_eval or "1"
    cli_variables['eval_split'] = args.eval_split or "dev"
