"""
MiniOneRec SFT Training Pipeline - Azure ML SDK v2
在 A100 GPU 节点上克隆 MiniOneRec 仓库并执行 SFT pointwise 训练
...
minionerec_train_component = load_component(
    source=str(SCRIPT_DIR / "components" / "minionerec_train" / "component.yaml")
)
...
# ============================================================================
@dsl.pipeline(
    description="MiniOneRec SFT Pointwise Training Pipeline on MIND dataset",
)
def minionerec_train_pipeline(
    msndni_input: Input,
    ...
    """MiniOneRec SFT Training Pipeline
    ...
    train_node = minionerec_train_component(
        msndni=msndni_input,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="MiniOneRec SFT Training Pipeline 提交脚本")
    parser.add_argument("--experiment-name", default="minionerec_sft_training",
                        help="AML experiment 名称 (default: minionerec_sft_training)")
