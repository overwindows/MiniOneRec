...
do
    ...
    exp_name="output_dir/sft_text_mixed_Industrial_and_Scientific_Qwen3-1.7B_bs1024"
    ...
    # Similarity-based evaluation (default: enabled)
    # Set USE_SIMILARITY=false for exact matching
    # Threshold is not used — evaluator always selects the most similar item.
    USE_SIMILARITY=${USE_SIMILARITY:-true}

    echo "Calculating metrics with similarity matching (no threshold)..."
