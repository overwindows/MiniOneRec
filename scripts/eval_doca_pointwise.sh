#!/usr/bin/env bash
# DOCA Point-wise Evaluation Script
#
# USAGE EXAMPLES:
# ===============
#
# 1. Quick test (500 feeds, single GPU):
#    CUDA_VISIBLE_DEVICES=0 bash scripts/eval_doca_pointwise.sh output_dir/final_checkpoint
#
# 2. Full dev evaluation (single GPU):
#    CUDA_VISIBLE_DEVICES=0 bash scripts/eval_doca_pointwise.sh output_dir/final_checkpoint --all
#
# 3. Parallel multi-GPU evaluation:
#    CUDA_VISIBLE_DEVICES=0,1,2,3 bash scripts/eval_doca_pointwise.sh output_dir/final_checkpoint
#
# 4. Full evaluation with 8 GPUs:
#    CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 bash scripts/eval_doca_pointwise.sh output_dir/final_checkpoint --all
#
# 5. Custom data path:
#    EVAL_JSONL=/path/to/dev.jsonl CUDA_VISIBLE_DEVICES=0,1,2,3 bash scripts/eval_doca_pointwise.sh model
#
...
echo "========================================="
echo "DOCA Point-wise Evaluation"
echo "========================================="
...
if [[ ! -f "${EVAL_JSONL}" ]]; then
  ...
  echo "Run: python src/prepare_doca.py --output_dir data/doca" >&2
  exit 1
fi
...
# Common eval args
COMMON_ARGS="--model_path \"${MODEL_PATH}\" --batch_size ${BATCH_SIZE} --flash_attn --use_chat_template"
if [[ "${MAX_FEEDS}" -gt 0 ]]; then
  COMMON_ARGS="${COMMON_ARGS} --max_feeds"
fi
...
if [[ "${PARALLEL_MODE}" == "true" ]]; then
  ...
  for gpu_id in ${cudalist}; do
    ...
    cmd="CUDA_VISIBLE_DEVICES=$gpu_id python -u src/evaluate_doca_pointwise.py \
  ...
  # Merge results
  OUTPUT_FILE="./results_doca/dev_scores.txt"
  ...
  echo ""
  echo "========================================="
  echo "✓ Parallel DOCA evaluation completed!"
  echo "========================================="
  ...
  CMD="python src/evaluate_doca_pointwise.py \
    --model_path \"${MODEL_PATH}\" \
    --eval_jsonl \"${EVAL_JSONL}\" \
    --batch_size ${BATCH_SIZE} \
    --flash_attn \
    --use_chat_template \
    --output_scores_file \"${OUTPUT_FILE}\""
  ...
  echo ""
  echo "========================================="
  echo "✓ DOCA evaluation completed!"
  echo "========================================="
  echo "Scores saved to: ${OUTPUT_FILE}"
fi
