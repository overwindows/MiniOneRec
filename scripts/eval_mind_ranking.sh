#!/usr/bin/env bash
# MIND Evaluation with Ranking-Aware Format (Multi-GPU Support)
#
...
#   CUDA_VISIBLE_DEVICES=0 bash scripts/eval_mind_ranking.sh output_dir/sft_mind_ranking_*/final_checkpoint dev
#
#   # Multi-GPU parallel evaluation (4-8x faster)
#   CUDA_VISIBLE_DEVICES=0,1,2,3 bash scripts/eval_mind_ranking.sh output_dir/sft_mind_ranking_*/final_checkpoint dev
#
#   # With abstracts
#   USE_ABSTRACT=1 CUDA_VISIBLE_DEVICES=0,1,2,3 bash scripts/eval_mind_ranking.sh output_dir/sft_mind_ranking_*/final_checkpoint dev
...
if [[ "${PARALLEL_MODE}" == "true" ]]; then
  ...
  TEMP_DIR="./temp_mind_ranking/${SPLIT}-$$"
  mkdir -p "${TEMP_DIR}"
  ...
  OUTPUT_FILE="${OUTPUT_FILE:-./results_mind/${SPLIT}_ranking_predictions.txt}"
